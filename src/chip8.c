...
static void chip8_execute_extended_eight(struct chip8 *chip8, unsigned short opcode)
{
...
static void chip8_exec_extended(struct chip8 *chip8, unsigned short opcode)
{
    ...
    // >> 8 is used because f is in btye1 and we need to move this to the
    // beginning. so that it would become 00 0f (example 0x0200 -> 0x02 (u8) or 0x0002 (u16))
    // unsigned char x = (opcode >> 8) & 0x0f00;
    // unsigned char y = (opcode >> 4) & 0x00f0;
    unsigned char x = (opcode >> 8) & 0x000f;
    unsigned char y = (opcode >> 4) & 0x000f;
    ...
    // CALL addr, 2nnn call subroutine at loction nnn
    case 0x2000:
    ...
    // 5xy0 = SE, Vx, Vy, skip the next instruction if Vx == Yy
    case 0x5000:
        if (chip8->registers.V[x] == chip8->registers.V[y])
        {
            chip8->registers.PC += 2;
    ...
    // 8xy0
    case 0x8000:
        chip8_execute_extended_eight(chip8, opcode);
        break;
